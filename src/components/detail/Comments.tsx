...
const Comments = ({ post_id }: { post_id: string }) => {
    ...
    if (event.key === "Enter" && inputText.trim() !== "") {
      ...
      if (currentLoginUserInfo) {
        ...
        const result = await addComment(
          ...
          nickname,
          avatar_img
        ); // 지금 이 부분 문제있음. 콘솔로 result찍어보니 null 나옴
  ...
  // 댓글 삭제
  const handleDeleteComment = async (comment_id: any) => {
    const currentLoginUserInfo = await getCurrentLoginUserInfo();
    if (!currentLoginUserInfo) {
  ...
  return (
    <div className="flex-col mt-28">
      <div className="flex">
        ...
        <p className="text-deepgrey text-2xl font-bold ml-2">
          {comments.length}
        </p>
      </div>
      <div>
              ...
              <div key={comment.comment_id} className="m-5 h-54">
                <div className="flex items-center mt-5 justify-between">
                  <div className="flex items-center">
                    ...
                    />
                    <p className="text-black text-xl font-bold ml-5">
                      {comment.nickname}
                    </p>
                  </div>
                ...
                {editingCommentId === comment.comment_id ? (
                  <div>
                    ...
                    <div className="flex justify-end ml-10">
                      <button
                        className="mr-5 text-lg border-2 w-20 rounded-full ring-2 hover:bg-peach hover:text-black hover:ring-0"
                  ...
                  <div>
                    ...
                    <div className="flex justify-end ml-10">
                      {currentUserId === comment.user_id && (
                        <div>
                          <button
                            className="mr-5 text-lg border-2 w-20 rounded-full ring-4 ring-purple-100 hover:bg-peach hover:text-black hover:ring-0"
                            onClick={() =>
                              startEditComment(
                                comment.comment_id,
                                comment.content
                          ...
                          <button
                            className="mr-5 text-lg border-2 w-20 rounded-full text-red-500 ring-4 ring-purple-100 hover:bg-peach hover:text-red hover:ring-0"
                            onClick={() =>
